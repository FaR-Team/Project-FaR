...
{
    public class FaRHierarchyPaletteWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            void colors()
            {
                ...
                void color(int i)
                {
                    ...
                    void color()
                    {
                        ...
                        if (i <= FaRHierarchyPalette.greyColorsCount)
                        {
                        ...
                        var colorRaw = palette ? palette.colors[i - 1] : FaRHierarchyPalette.GetDefaultColor(i - 1);
                        ...
                        if (i > FaRHierarchyPalette.greyColorsCount && isDarkTheme)
                            color *= 1.41f;
            ...
            FaRHierarchy.goInfoCache.Clear();
        ...
        bool usingDataSO => !gameObjects.Select(r => r.scene).All(r => FaRHierarchy.dataComponents_byScene.GetValueOrDefault(r) != null);
        IEnumerable<FaRHierarchyDataComponent> usedDataComponents => FaRHierarchy.dataComponents_byScene.Where(kvp => kvp.Value && gameObjects.Select(r => r.scene).Contains(kvp.Key)).Select(kvp => kvp.Value);
        ...
        static void RepaintOnAlt() // Update 
        {
            if (curEvent.holdingAlt != wasHoldingAlt)
                if (EditorWindow.mouseOverWindow is FaRHierarchyPaletteWindow paletteWindow)
        ...
        public void Init(List<GameObject> gameObjects)
        {
            void createData()
            {
                if (FaRHierarchy.data) return;

                FaRHierarchy.data = ScriptableObject.CreateInstance<FaRHierarchyData>();

                AssetDatabase.CreateAsset(FaRHierarchy.data, GetScriptPath("FaRHierarchy").GetParentPath().CombinePath("FaRHierarchy Data.asset"));

            }
            void createPalette()
            {
                if (FaRHierarchy.palette) return;

                FaRHierarchy.palette = ScriptableObject.CreateInstance<FaRHierarchyPalette>();

                AssetDatabase.CreateAsset(FaRHierarchy.palette, GetScriptPath("FaRHierarchy").GetParentPath().CombinePath("FaRHierarchy Palette.asset"));
            ...
            void getInfos()
            {
                ...
                foreach (var r in gameObjects)
                    goDatas.Add(FaRHierarchy.GetGameObjectData(r, createDataIfDoesntExist: true));
        ...
        static FaRHierarchyPalette palette => FaRHierarchy.palette;
        static FaRHierarchyData data => FaRHierarchy.data;
        ...
        {
            instance = ScriptableObject.CreateInstance<FaRHierarchyPaletteWindow>();
        ...
        public static FaRHierarchyPaletteWindow instance;
