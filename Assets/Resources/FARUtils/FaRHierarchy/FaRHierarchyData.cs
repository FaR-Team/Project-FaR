...
{
    public class FaRHierarchyData : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        public void OnBeforeSerialize() => FaRHierarchy.OnDataSerialization();
                ...
                get
                {
                    if (_go == null)
                        if (!failedToLoadSceneObject) // to prevent continuous GlobalID.GetObjects() calls if object is deleted
                            FaRHierarchy.unloadedSceneBookmarks_sceneGuids.Add(globalId.guid);
        ...
        [CustomEditor(typeof(FaRHierarchyData))]
        class Editor : UnityEditor.Editor
        {
            public override void OnInspectorGUI()
            {
                ...
                void teamMode()
                {
                    ...
                    Space(10);
                    EditorGUILayout.LabelField("Now that Team Mode is enabled, create an empty script that inherits from FaRHierarchy.FaRHierarchyDataComponent and add it to any object in a scene.", style);
        ...
        public static bool teamModeEnabled { get => EditorPrefsCached.GetBool("FaRHierarchy-teamModeEnabled", false); set => EditorPrefsCached.SetBool("FaRHierarchy-teamModeEnabled", value); }
        ...
        public void EnableTeamMode()
        {
            var option = EditorUtility.DisplayDialogComplex("Licensing notice",
                                                            "To use FaRHierarchy 2 within a team, licenses must be purchased for each individual user as per the Asset Store EULA.\n\n Sharing one license across the team is illegal and considered piracy.",
                                                            "Acknowledge",
                                                            "Cancel",
                                                            "Purchase more seats");
            if (option == 2)
            ...
            FaRHierarchy.goDataCache.Clear();
