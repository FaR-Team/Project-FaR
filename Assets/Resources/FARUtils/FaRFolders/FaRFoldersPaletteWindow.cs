...
using static FaRFolders.FaRFolders;
using static FaRFolders.FaRFoldersData;
using static FaRFolders.FaRFoldersPalette;
using static FaRFolders.FaRFoldersCache;


namespace FaRFolders
{
    public class FaRFoldersPaletteWindow : EditorWindow
    {
        void OnGUI()
        {
        ...
        {
            if (FaRFoldersData.storeDataInMetaFiles) return; // empties removed from meta files in SaveData()
        ...
        {
            if (!FaRFoldersData.storeDataInMetaFiles)
                if (data)
                    data.RecordUndo();
        ...
        void MarkDatasDirty()
        {
            ...
            if (FaRFoldersData.storeDataInMetaFiles)
                FaRFolders.folderDatasFromMetaFiles_byGuid.Clear();

        }
        void SaveData()
        {
            if (!FaRFoldersData.storeDataInMetaFiles) { data.Save(); return; }
        ...
        public void Init(List<string> guids)
        {
            void createData()
            {
                if (FaRFolders.data) return;

                FaRFolders.data = ScriptableObject.CreateInstance<FaRFoldersData>();

                AssetDatabase.CreateAsset(FaRFolders.data, GetScriptPath("FaRFolders").GetParentPath().CombinePath("FaRFolders Data.asset"));

            }
            void createPalette()
            {
                if (FaRFolders.palette) return;

                FaRFolders.palette = ScriptableObject.CreateInstance<FaRFoldersPalette>();

                AssetDatabase.CreateAsset(FaRFolders.palette, GetScriptPath("FaRFolders").GetParentPath().CombinePath("FaRFolders Palette.asset"));
            ...
            void getInfos()
            {
                ...
                foreach (var r in guids)
                    folderInfos.Add(FaRFolders.GetFolderInfo(r, createDataIfDoesntExist: true));
        ...
        static FaRFoldersPalette palette => FaRFolders.palette;
        static FaRFoldersData data => FaRFolders.data;
        ...
        {
            instance = ScriptableObject.CreateInstance<FaRFoldersPaletteWindow>();
        ...
        public static FaRFoldersPaletteWindow instance;
