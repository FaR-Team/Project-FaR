...
#pragma warning disable 0649
		...
		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, la consola va a mantenerse entre escenas" )]
		private bool singleton = true;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Altura mínima de la ventana de la consola" )]
		private float minimumHeight = 200f;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, La ventana de la consola puede también agrandarse horizontalmente" )]
		private bool enableHorizontalResizing = false;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, el botón de cambiar tamaño de la consola va a estar en la esquina inferior derecha. si no, va a estar en la esquina inferior izquierda" )]
		private bool resizeFromRight = true;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Ancho mínimo de la ventana de la consola" )]
		private float minimumWidth = 240f;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si no está activo, No se mostrará el popup cuando la consola esté oculta" )]
		private bool enablePopup = true;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, la consola se va a iniciar como popup" )]
		private bool startInPopupMode = true;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, la consola va a ser invisible inicialmente" )]
		private bool startMinimized = false;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, apretar la tecla designada va amostrar/ocultar la consola" )]
		private bool toggleWithKey = false;

#if ENABLE_INPUT_SYSTEM && !ENABLE_LEGACY_INPUT_MANAGER
		[SerializeField]
		[HideInInspector]
		public InputAction toggleBinding = new InputAction( "Tecla Designada", type: InputActionType.Button, binding: "<Keyboard>/backquote", expectedControlType: "Button" );
#else
		[SerializeField]
		[HideInInspector]
		...
		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, la ventana de la consola va a tener una barra de búsqueda" )]
		...
		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, la ventana de la consola va a continuar recibiendo Logs en el fondo incluso si el gameobject está inactivo." )]
		...
		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, los tiempos de llegada de logs se van a recordar y mostrar cuando se expande un log" )]
		private bool captureLogTimestamps = false;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, las timestamps se van a mostrar para logs aunque noe stén expandidos" )]
		internal bool alwaysDisplayTimestamps = false;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Mientras se oculte la consola, los log que lleguen se guardan en cola pero no se procesan inmediatamente hasta que la ventana de la consola se abra. Cuando la cola de logs excede el límite, el primer logs de la cola va a ser procesado para reforzar el límite." )]
		private int queuedLogLimit = 256;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, el campo de entrada de comandos en la parte inferior de la consola se va a vaciar automáticamente tras poner un comando" )]
		private bool clearCommandAfterExecution = true;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "La consola se guarda todos los comandos introducidos previamente. Este valor determina la capacidad del historial de comandos" )]
		private int commandHistorySize = 15;

		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, mientras escribas un comando, todos los comandos que coincidan se van a mostrar en popup" )]
...
#pragma warning disable 0169
		...
		[SerializeField]
		[HideInInspector]
		[Tooltip( "Los logs nativos van a ser filtrados usando estos arguementos. Si se dejan en blanco, todos los logs nativos de la aplicación se van a loggear a la consola." )]
...
#pragma warning restore 0414
		...
		[SerializeField]
		[Tooltip( "Si un log es más largo que este límite, se va a cortar. Esto ayuda a llegar al límite de 65000 vertices de unity" )]
		private int maxLogLength = 10000;

#if UNITY_EDITOR || UNITY_STANDALONE || UNITY_WEBGL
		[SerializeField]
		[HideInInspector]
		[Tooltip( "Si está activo, se va a centrar el input directamente tras abrir la consola" )]
...
#pragma warning restore 0649
		...
		private void OnEnable()
		{
			...
			DebugLogConsole.AddCommand( "logs.save", "Guarda logs a PersistentDataPath", SaveLogsToFile );
			DebugLogConsole.AddCommand<string>( "logs.save", "Guarda logs a un archivo especificado", SaveLogsToFile );
